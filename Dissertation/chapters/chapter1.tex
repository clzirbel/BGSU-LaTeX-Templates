...
The easiest way to cite published works is to look them up with an online tool like\\ \noindent scholar.google.com, copy the BibTeX citation information into the file reference.bib (click the double quotes under the article in Google Scholar), then run BibTex on dissertation.tex.
...
Some articles have more than two authors, and it's nice if the author list is not so long in the text.
When there are 3 or more authors, \LaTeX~is supposed to be able to show just the first author and say ``et al.'' for the rest.
But that is not always working exactly the way I would expect!
Here are some examples, hopefully you can find something that works.
This is what we get with cite: \cite{test123}.
This is what we get with citet: \citet{test123}.
This is what we get with citep: \citep{test123}.
This is what we get with citeauthor*:  \citeauthor*{test123}.
...
There is a page break here to make it easier to start a screen reader to hear how it reads the equations on the next page.
...
Note that when you are reading the PDF version of this document and you click a link to take you to an equation or a reference, you can press Left Alt plus the left arrow key to get back to where you were.
That's pretty arcane, but it sure helps you navigate a PDF.
