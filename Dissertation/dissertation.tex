...
%
%    Another note: Latex is currently working on a solution regarding making
%    accessible documents. At this point in time, tagging a document and
%    unrecognized unicode characters are two common errors in an accessibility 
%    check with Adobe Pro. There is no known solutions, but some have suggested
%    compiling with LuaLaTeX or XeLaTeX instead of pdfLaTeX. If you are using 
%    Overleaf, this can be done by going to Menu (upper left corner), then changing
%    the Compiler.
...
%\bibliographystyle{apalike}   %% sometimes works better than chicago, especially for 3 or more authors
\setcitestyle{authoryear, open={((},close={))}}
% use the file reference.bib
\bibliography{reference}  % use the file reference.bib
